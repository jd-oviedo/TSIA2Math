...
const MISC = [
  ...
  { rank: 5, strand: 'PR', topic: 'PR.1.5', freq: 7,  students: 4, text: 'Student reasons that there are only two possible outcomes, confusing a binary categorical variable with a discrete numerical variable.', trend: [1,1,1,2,1,1] },
...
ORDER.forEach(k => { if (strandPct[k] < strandPct[ws]) ws = k; });
  ...
  return (
    <>
      <style>{`* { box-sizing: border-box; } body { margin: 0; background: #F5F5F3; -webkit-font-smoothing: antialiased; }`}</style>

      {/* Demo banner */}
      <div style={{ position: 'sticky', top: 0, zIndex: 100, background: '#0F1E35', color: '#fff', padding: '11px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 13 }}>
        <span>📊 This is a live demo with sample data — no real student information is shown.</span>
        <a href="/login?role=teacher" style={{ background: '#C68A2F', color: '#fff', padding: '7px 18px', borderRadius: 8, fontWeight: 700, fontSize: 13, textDecoration: 'none', whiteSpace: 'nowrap' }}>Get started free →</a>
      </div>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', minWidth: 1280, fontFamily: 'Arial, Helvetica, sans-serif', color: '#1A1A1A' }}>
        ...
        <aside style={{ width: 200, flex: '0 0 200px', background: '#0F1E35', color: '#fff', display: 'flex', flexDirection: 'column', position: 'sticky', top: 44, height: 'calc(100vh - 44px)' }}>
          ...
          <nav style={{ padding: '6px 12px', display: 'flex', flexDirection: 'column', gap: 2 }}>
            {[
              ...
              { label: 'Take a practice test', active: false },
              { label: 'Student view', active: false },
            ].map(item => (
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '9px 11px', borderRadius: 8, fontSize: 13, fontWeight: item.active ? 600 : 500, color: item.active ? '#E7BE7B' : 'rgba(255,255,255,0.64)', background: item.active ? 'rgba(198,138,47,0.14)' : 'transparent', boxShadow: item.active ? 'inset 3px 0 0 #C68A2F' : 'none' }}>
                {item.label}
              </div>
            ))}
          </nav>
        ...
        <main style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', background: '#F5F5F3' }}>
          ...
          {/* Top bar */}
          <header style={{ height: 60, flex: '0 0 60px', background: '#fff', borderBottom: '1px solid rgba(15,30,53,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px', position: 'sticky', top: 44, zIndex: 5 }}>
          ...
          <div style={{ padding: '26px 32px 52px 32px' }}>
            ...
            <div style={{ marginBottom: 22 }}>
              <h1 style={{ margin: 0, fontFamily: "'Kodchasan',sans-serif", fontWeight: 600, fontSize: 27, letterSpacing: -0.4, color: '#0F1E35' }}>TSIA2 Prep — Period 2</h1>
            ...
            <div style={{ background: '#fff', border: '1px solid rgba(15,30,53,0.07)', borderRadius: 12, overflow: 'hidden', marginBottom: 34 }}>
                ...
                <thead>
                  <tr style={{ background: '#FBFBF9', borderBottom: '1px solid #E7E5DD' }}>
                    {['Student','Score','Placement','Strand profile','Tests','Last active'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: h === 'Student' ? '11px 20px' : '11px 14px', fontSize: 11, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase', color: '#8A8983' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16 }}>
              {misc.map(m => (
                <div key={m.rank} style={{ background: '#fff', border: '1px solid rgba(15,30,53,0.07)', borderRadius: 12, padding: 18, boxShadow: '0 1px 2px rgba(15,30,53,0.04)', display: 'flex', flexDirection: 'column', minHeight: 182 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 14, lineHeight: 1.5, color: '#26262A', flex: '1 1 auto' }}>{m.text}</p>
            ...
            {/* Bottom CTA */}
            <div style={{ marginTop: 48, background: '#0F1E35', borderRadius: 16, padding: '36px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
              ...
              <a href="/login?role=teacher" style={{ background: '#C68A2F', color: '#fff', padding: '14px 28px', borderRadius: 10, fontWeight: 700, fontSize: 15, textDecoration: 'none', whiteSpace: 'nowrap', flexShrink: 0 }}>Get started free →</a>
            </div>
