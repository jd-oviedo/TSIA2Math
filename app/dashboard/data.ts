 ...
 * showsClassChrome below adds a third caller in the layout. Memoised per
 * request they are one read; the resolver in course-access.ts is cache()d for
 * exactly this reason and this is the same shape. PER REQUEST ONLY -- a student
 * who joins a class must see it on their next page load.
 */
 ...
 *
 * THE ONE PLACE THIS QUESTION IS ANSWERED. The rail, Home's join affordance and
 * the two class-fed pages all call this, so the nav a student is given and the
 * pages behind it cannot come apart -- a link that leads somewhere structurally
 * empty, or an empty state on a page whose link was hidden, would both be that
 * drift.
 *
